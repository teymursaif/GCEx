...
def assess_figures_GC_simulations(gal_id):
    ...
    DET_ART_GCs_cat = art_dir+gal_name+'_'+fn_det+'_DET_ART_GCs.fits'
    ...
    for fn in filters:
        ...
        if fn == fn_det:
            ...
            ax[2][n_filter].set_ylim([0,15])
            ax[2][n_filter].legend(loc='upper left')
            ax[2][n_filter].tick_params(which='both',direction='in')


        if fn == fn_det:
            param1 = det_art_gcs['CI_4_8_'+fn] #+ det_art_gcs['CI_6_8_'+fn]
            param2 = det_art_gcs['MAG_AUTO_'+fn]
            param3 = sources['CI_4_8_'+fn] #+ sources['CI_6_8_'+fn]
            param4 = sources['F_MAG_APER_CORR_'+fn]
            ax[2][n_filter+1].scatter(param1,param2,s=50,color=colors[n_filter],alpha=0.1,marker='s',label='Detected Artificial GCs')
        ...
        if fn == fn_det:
            ...
            ax[3][n_filter].hist(mag_source_sim,bins=np.arange(10,30,0.5),color='black',alpha=1,label='ALL simulated GCs')
            ax[3][n_filter].hist(mag_source_obs,bins=np.arange(10,30,0.5),color=colors[n_filter],alpha=1,label='Detected simulated GCs')
            mag1 = GC_MAG_RANGE[0]+5*np.log10(distance*1e+5)-0.25+color(fn,fn_det)
            mag2 = GC_MAG_RANGE[1]+5*np.log10(distance*1e+5)+0.25+color(fn,fn_det)
        ...
        if fn == fn_det:
            ...
            mag_source_sim = art_gcs['GC_MAG_'+fn]
            r_source_sim = np.sqrt((art_gcs['RA']-ra)**2 + (art_gcs['DEC']-dec)**2)*3600
            r_source_obs = np.sqrt((det_art_gcs['RA_1']-ra)**2 + (det_art_gcs['DEC_1']-dec)**2)*3600
