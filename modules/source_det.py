...
def prepare_sex_cat(source_cat_name_input,source_cat_name_output,gal_name,filter_name,distance):
    ...
    expand_fits_table(source_cat_name_output,'MAG_APER_80percent',mag_apers[:,0])
    expand_fits_table(source_cat_name_output,'MAGERR_APER_80percent',magerr_apers[:,0])

    expand_fits_table(source_cat_name_output,'FLUX_APER_80percent',flux_apers[:,0])
    expand_fits_table(source_cat_name_output,'FLUXERR_APER_80percent',fluxerr_apers[:,0])

    expand_fits_table(source_cat_name_output,'APER_RAD_80percent_PIXEL',mag_apers[:,0]*0+float(apertures[0]))
...
def make_mask(frame, sex_source_cat, weight, mask_out, mask_out2, weight_out, flag_out, ra, dec, mode='cat'):
    ...
    if mode == 'cat' :
        ...
        B = table['B_IMAGE']
        for i in range (0,N) :
            r = math.sqrt((ra-ra_star[i])**2+(dec-dec_star[i])**2)
            if flag[i] <= 9999 and r >= 0.0001/3600.:
                if x[i] >= X or y[i] >= Y :
                    continue
                x0 = int(x[i])
                y0 = int(y[i])
                #mask_size = int(fwhm[i])
                mask_size = int(3*B[i])
...
def make_detection_frame(gal_id, input_frame, output_frame, backsize=16, backfiltersize=1, iteration=3):
    ...
    for i in range(0,iteration):
        ...
        if i == 0 :
            ...
            weight_command = '-WEIGHT_TYPE BACKGROUND -WEIGHT_IMAGE '+temp_dir+'temp_weight'+str(i)+'.fits -WEIGHT_THRESH 1.0 '

        command = 'sex '+temp_dir+'temp_det.fits'+' -c '+str(input_dir)+'default.sex -CATALOG_NAME '+temp_dir+'temp_sex_cat.fits'+str(i)+'.fits '+ \
        '-PARAMETERS_NAME '+str(input_dir)+'default.param -DETECT_MINAREA 4 -DETECT_THRESH 3.0 -ANALYSIS_THRESH 3.0 ' + \
        ...
        make_mask(input_frame,temp_dir+'temp_sex_cat.fits'+str(i)+'.fits',None,temp_dir+'temp_mask'+str(i+1)+'.fits',temp_dir+'temp_mask+'+str(i+1)+'.fits',temp_dir+'temp_weight'+str(i+1)+'.fits',\
    ...
    os.system('cp '+temp_dir+'temp_mask'+str(iteration-1)+'.fits'+' '+sex_dir+gal_name+'_'+'mask'+'_cropped.fits')
...
def make_source_cat(gal_id):
    ...
    for fn in filters:
        ...
        make_detection_frame(gal_id,data_dir+gal_name+'_'+fn+'_cropped.fits',output_frame=detection_frame)
        ...
        '-BACK_SIZE 32 -BACK_FILTERSIZE 1 -CHECKIMAGE_TYPE APERTURES,FILTERED,BACKGROUND,-BACKGROUND,SEGMENTATION ' +  \
...
def make_multiwavelength_cat(gal_id, mode='forced-photometry'):
    ...
    if mode=='cross-match' :
        ...
        for fn in filters[1:]:
            ...
            crossmatch('join.fits',cat,'RA','DEC','RA_'+fn,'DEC_'+fn,0.1,fn,'join.fits')

        os.system('mv join.fits '+cats_dir+gal_name+'_master_cat.fits')

    elif mode=='forced-photometry':
        ...
        for fn in filters:
            ...
            forced_photometry(det_cat, photom_frame, fn, output)

        os.system('mv '+output+' '+cats_dir+gal_name+'_master_cat.fits')

############################################################

def forced_photometry(det_cat, photom_frame, fn, output):
    ...
    for i in range(N_objects):
        ...
        if (x<1) or (y<1) or (x>X-1) or (y>Y-1):
            ...
            continue
        #apers = PHOTOM_APERS.split(',')
        #for aper_size in apers[:1]:
        aper_size = ((PSF_REF_RAD_ARCSEC[fn])[0])/PIXEL_SCALES[fn]
        #print (aper_size, aper_size)
        #aper_size = float(aper_size)
        aper = CircularAperture((x, y), aper_size)
        sky_aper = CircularAnnulus((x, y), aper_size*3, aper_size*10)
        aper_area = aper.area_overlap(data=fits_data,method='exact')
        sky_area = sky_aper.area_overlap(data=fits_data,method='exact')
        #print (aper_area, sky_area, sky_area/aper_area)

        try :
            flux, flux_err = aper.do_photometry(data=fits_data,method='exact')
            sky_flux, flux_err = sky_aper.do_photometry(data=fits_data,method='exact')
            flux = float(flux[0])-float(sky_flux[0])/(sky_area/aper_area)
            flux_total = (flux) / (PSF_REF_RAD_FRAC[fn])[0]
            mag = -2.5*np.log10((flux_total))+zp
            mag_err = 0
            MAG.append(mag)
            MAG_ERR.append(mag_err)
            print ('- magnitude for object is:', mag)
        except:
            #print (' something went wrong for this object ... ')
            mag = -99
            mag_err = -99
            MAG.append(mag)
            MAG_ERR.append(mag_err)
