...
def initialize_params() :
    ...
    WORKING_DIR = './'
    main_data_dir = WORKING_DIR+'ERO-data/ERO-GC/'
    ...
    INPUT_EXPTIME = {'VIS':560,'NISP-Y':110,'NISP-J':110,'NISP-H':110}
    ...
    frame = main_data_dir+'/ERO-NGC6254_VIS.fits'
    ...
    for j in range(N):
        for i in range(N):
            ...
            if m == 1:
                target_str = str(m)+' ERO-NGC6254 ERO-NGC6254-'+str(m)+' '+str(ra)+' '+str(dec)+' 100 VIS,NISP-H MAKE_CAT,SIM_GC,MAKE_GC_CAT ---' #MAKE_CAT,SIM_GC, ,NISP-Y,NISP-J,NISP-H
                TARGETS.append([target_str])
            else:    
                target_str = str(m)+' ERO-NGC6254 ERO-NGC6254-'+str(m)+' '+str(ra)+' '+str(dec)+' 100 VIS,NISP-H --- ---' #MAKE_CAT,SIM_GC, ,NISP-Y,NISP-J,NISP-H
                TARGETS.append([target_str])
            #print (target_str)
    ...
    PARAM_SEL_METHOD = 'OFF'#'MANUAL'
    PARAM_SEL_RANGE = {'ELLIPTICITY':[0,0.4],'MAG_APER_CORR':[21,25],'F_MAG_APER_CORR':[21,25],'F_MAG_APER_CORR_NISP-Y':[15,30],\
    'color0':['VIS','VIS',-0.5,0.5],'color1':['VIS','NISP-Y',-1.0,1.0],'color2':['NISP-Y','NISP-J',-1,1],'color3':['NISP-J','NISP-H',-1,1]} #\
