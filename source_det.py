...
def make_source_cat(gal_id):
    ...
    for fn in filters:
        ...
        '-DEBLEND_NTHRESH 4 -DEBLEND_MINCONT 0.005 ' + weight_command + ' -PHOT_APERTURES '+str(psf_dia_ref_pixel)+','+str(PHOTOM_APERS)+' -GAIN ' + str(gain) + ' ' \
        ...
        '-BACK_SIZE 64 -BACK_FILTERSIZE 1 -CHECKIMAGE_TYPE APERTURES,FILTERED,BACKGROUND,-BACKGROUND,SEGMENTATION,BACKGROUND_RMS ' +  \
...
def make_source_cat_for_sim(gal_id):
    ...
    for n in range(N_SIM_GCS) :
        ...
        if 'USE_SUB_GAL' in methods:
            for fn in filters:
                shutil.copy(fit_dir+gal_name_orig+'_'+fn+'_galfit_imgblock_res.fits',fit_dir+gal_name+'_'+fn+'_galfit_imgblock_res.fits')
